...
//에러나면 패키지 우클릭 -> Build Path -> Configure Build Path -> Add Library - > Server Runtime -> Tomcat 9.0 피니쉬
//런해서 에러나는건 값을 안줘서.
//jsp 처리를 제대로 안해서 위에 이상한게 나오긴 함.


//다음 강의에서 설명할건데, 스프링의 @Controller + @ RequestMapiping 두개 합친거.
//서블릿은 메서드 단위로 매핑이 안돼고 클래스 단위로만 매핑이 가능하다
//그래서 서블릿은 스프링에 비해 클래스를 많이 만들어야 되는 단점이 있다.
...
public class MyDispatcherServlet extends HttpServlet {
	@Override
	//서블릿은 메서드는 꼭 service라는 이름으로 해야 된다. 매개변수도 request, response 꼭 두개를 적어줘야 된다.
